...
- (UISearchBar *)searchBar{
    ...
    if (!_searchBar) {
        _searchBar=[[UISearchBar alloc]initWithFrame:CGRectMake(0, 64, KCWIDTH, 44)];
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    [ud setObject:cell.textLabel.text forKey:@"city"];
    [ud setObject:@"第二次" forKey:@"是"];
    [ud synchronize];
    self.block(cell.textLabel.text);
    
    NSDictionary *dict = [[NSDictionary alloc] initWithObjectsAndKeys:cell.textLabel.text,@"text",nil];
