...
-(void)initUI
{
    ...
    //_tfUserPass.borderStyle = UITextBorderStyleRoundedRect;
    _tfUserPass.clearButtonMode = UITextFieldViewModeAlways;
    _tfUserPass.delegate = self;
    _tfUserPass.secureTextEntry = YES;
   ...
   // _tfCodePass.borderStyle = UITextBorderStyleRoundedRect;
    ...
    [pswSeebutton mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.right.equalTo(pswView.mas_right).equalTo(-12);
        make.width.equalTo(view2.mas_width).multipliedBy(0.04);
        make.height.equalTo(view2.mas_height).multipliedBy(0.06);
    }];
    ...
    [_selectButton setTitleColor:RGB(38, 107, 255) forState:UIControlStateNormal];
...
-(void)registButtonClick {
        ...
        NSString *url = [NSString stringWithFormat:@"%@/app/patient/index/register",KCBaseURLString];
        ...
        [manager POST:url parameters:parameters progress:^(NSProgress * _Nonnull uploadProgress) {
            ...
            if ([[responseObject objectForKey:@"MSG"] isEqualToString:@"注册成功"] && _selectButton.selected == YES) {
                
                if (self.block != nil) {
                    self.block(_tfUserPhone.text, _tfUserPass.text);
                }
...
-(void)selectbuttonClick
{
    ...
    if (_selectButton.selected == YES) {
        ...
        [_registerButton setBackgroundColor:RGB(38, 107, 255)];
    }else
    {
