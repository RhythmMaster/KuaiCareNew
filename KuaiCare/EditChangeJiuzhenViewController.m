//
//  EditChangeJiuzhenViewController.m
//  KuaiCare
//
...
@interface EditChangeJiuzhenViewController ()<UITextFieldDelegate,ChangeStatus>
{
...
@implementation EditChangeJiuzhenViewController
...
- (void)viewDidLoad {
    ...
    NSString *url = [NSString stringWithFormat:@"%@/app/patient/mycenter/seedoctorpersonView",KCBaseURLString];
    ...
    [manager POST:url parameters:paramters progress:^(NSProgress * _Nonnull uploadProgress) {
        ...
        self.textfield5.text = [self.dict objectForKey:@"medicareCard"];
        self.xingbie = [self.dict objectForKey:@"sex"];
...
-(void)initUI
{
    ...
    for (int  i = 0; i<6 ; i++) {
        ...
        if (i == 0) {
            ...
            contactButton.frame = CGRectMake(20+KCWIDTH*0.8, 52+45*i, KCWIDTH*0.08, KCWIDTH*0.08);
            [contactButton addTarget:self action:@selector(contactButtonClick) forControlEvents:UIControlEventTouchUpInside];
            
            [contactButton setBackgroundImage:[UIImage imageNamed:@"icon_l_bt_tx"] forState:UIControlStateNormal];
        ...
        if (i == 1) {
            ...
            NSLog(@"%@",self.dict);
            textfield2.text = [self.dict objectForKey:@"tel"];
    ...
    for (int i = 4; i<7; i++) {
        ...
        if (i == 4) {
            ...
            textfiled.text = [self.dict objectForKey:@"birthday"];
            //self.textfield3.enabled = NO;
        }else if (i == 5)
        {
            self.textfield4 = textfiled;
            textfiled.text = [self.dict objectForKey:@"idcard"];
        }else
        {
            self.textfield5 = textfiled;
            textfiled.text = [self.dict objectForKey:@"medicareCard"];
        }
...
- (void)rightClick {
    ...
    NSDictionary *paramters = @{@"name":self.textfield1.text,@"tel":self.textfield2.text,@"sex":self.xingbie,@"birthday":self.textfield3.text,@"idcard":self.textfield4.text,@"medicareCard":self.textfield5.text,@"id":self.editID};
    NSLog(@"%@",paramters);
    AFHTTPSessionManager *manager = [AFHTTPSessionManager manager];
    [manager POST:url parameters:paramters progress:^(NSProgress * _Nonnull uploadProgress) {
