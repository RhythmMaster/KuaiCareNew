...
@interface EditViewController ()<UITextFieldDelegate,ChangeStatus, GYZChooseCityDelegate>
...
-(void)initUI {
    ...
    for (int  i = 0; i<5 ; i++) {
        ...
        if (i == 4) {
            UISwitch *switchView = [[UISwitch alloc] initWithFrame:CGRectMake(20+KCWIDTH*0.75, 12+45*i, KCWIDTH*0.2, KCWIDTH*0.1)];
            switchView.on = YES;
            self.isDefault = @"1";
...
- (void)rightClick {
    ...
    if (kAppdelegate.haveLogin) {
        ...
        NSDictionary *paramters = @{@"name":self.textfield1.text,@"tel":self.textfield2.text,@"address":self.textfield3.text,@"addressInfo":self.textfield4.text,@"isDefault":self.isDefault};
        NSLog(@"%@",paramters);
        ...
        if (self.textfield1.text.length == 0) {
            [self.view showWarning:@"请填写姓名"];
            return;
        }
        if (self.textfield2.text.length == 0) {
            [self.view showWarning:@"请填写电话号码"];
            return;
        }
        if (self.textfield4.text.length == 0) {
            [self.view showWarning:@"请填写详细地址"];
            return;
        }
        //self.noLoginBlock(self.textfield4.text, self.textfield1.text, self.textfield2.text);
    }
